...
void    checkPassword (User& new_user, std::string buffer, std::string password) {
...
void    setUsername (User& new_user, std::string buffer) {
...
int     getInfoUser (User& new_user, STRING_VECTOR bufferSplit, Server& irc_server) {
    ...
    if (((new_user.nickname == "" and checkDuplicateNick(bufferSplit[0], irc_server.users) == 1) || new_user.x >= 1) && new_user.x != 1000) {
        User john = findUser (bufferSplit[0], irc_server.users);
        if(new_user.x == 0)
        {
            if (john.online == true) {
        ...
        if (new_user.x == 1 || john.logInPwd != "") 
        {
            ...
            User john = findUser(new_user.nickname, irc_server.users);
    ...
    {
        print_message (new_user->sd, "Error: Your nickname must start with a letter.\n");
        return 1;
    }
