...
void    unban (User executer, STRING_VECTOR bufferSplit, Server& irc_server) {
    ...
    if(bufferSplit[1].empty () == 1 || bufferSplit[2].empty () == 1 || bufferSplit[3].empty () == 1)
        ...
        {
            print_message (executer.sd, "The chann : " + bufferSplit[1] + " you tried to unban from doesn't exist\n");
        }
        else
        {
			if (isOpe(kick_from->ope, executer) == 1)
			{
				print_message (executer.sd, "You don't have enought right to unban from the channel :" + bufferSplit[1] + "\n");
			}
